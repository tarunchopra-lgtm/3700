...
from alpaca.data.requests import CryptoTradesRequest, CryptoBarsRequest
...
try:
    credentials, trading_client = bootstrap_trading_auth("trend_long_crypto.py")
...
LOOKBACK = 10 # Lookback window for high/low breakout metrics
...
trade_request = CryptoTradesRequest(
symbol_or_symbols=SYMBOL,
start=start_date,
end=end_date
)
trades_data = data_client.get_crypto_trades(trade_request)
...
print(f"Columns available: {df_trades.columns.tolist()}")
print(f"Index name: {df_trades.index.name}")
...
# Reset index to use the original index (which contains timestamp)
custom_bars.index = df_trades.groupby('bar_group').apply(lambda x: x.index[0])
...
print(f"16-bar High (Entry breakout level): {highest_high}")
...
daily_data = data_client.get_crypto_bars(daily_request)
