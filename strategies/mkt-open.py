...
def main() -> int:
    """Main entry point."""
    # Handle help manually (before argparse for consistency)
    if len(sys.argv) > 1 and sys.argv[1] in ["--help", "-h", "help"]:
        print("""
MKT-OPEN.PY - Market Open Daily Breakout Trading Strategy

SYNTAX:
  python strategies/mkt-open.py [--interval SECONDS] [--help]

OPTIONAL:
  --interval SECONDS    Check interval (default: 20 seconds)
  --help, -h            Show this help message

DESCRIPTION:
  Automated market-open strategy for trading daily trend breakouts
  Identifies breakouts from previous trading day
  Executes trades at market open using calculated levels
  Simultaneously manages call options on breakout positions
  Monitors continuously for new breakout candidates

WORKFLOW:
  1. Load breakout candidates from lists/today-breakout
  2. For each symbol:
     - Get yesterday's high as entry reference
     - Calculate stop loss (2% below entry)
     - Calculate targets (3% and 5% above entry)
     - Buy NUM_STOCKS shares via fomo_trade.py
  3. Start gocall.py to manage call options on positions
  4. Monitor every INTERVAL seconds for new breakouts
  5. Add new breakouts and repeat steps 2-4

POSITION SIZING:
  - Shares per trade: 10 (configurable in code)
  - Stop loss: 2% below yesterday's high
  - Target 1: 3% above yesterday's high
  - Target 2: 5% above yesterday's high

EXECUTION:
  - Uses fomo_trade.py subprocess for each breakout
  - Splits position across multiple runs
  - Starts gocall.py for options automation
  - Tracks traded symbols to avoid duplicates

OPTIONS MANAGEMENT:
  - Automatically sells calls on breakout positions
  - Targets open interest minimum
  - Manages premiums and exit prices
  - Runs concurrently with stock trading

EXAMPLES:
  python strategies/mkt-open.py
    - Start strategy with default 20-second check interval
    - Load breakouts from lists/today-breakout

  python strategies/mkt-open.py --interval 10
    - Check for new breakouts every 10 seconds

  python strategies/mkt-open.py --interval 30
    - Check every 30 seconds (less frequent updates)

BREAKOUT FILE:
  Location: lists/today-breakout (no .txt suffix)
  Format: SYMBOL: Entry=X.XX Stop=X.XX Target=X.XX
  Source: Generated by find_daily_trend.py
  Usage: Loaded at startup, checked every interval

REQUIRED FILES/PROCESSES:
  - lists/today-breakout (generated by find_daily_trend.py)
  - strategies/fomo_trade.py (subprocess for entries)
  - strategies/gocall.py (subprocess for options)
  - Alpaca API credentials and live/paper account

MONITOR OUTPUT:
  - Config summary at startup
  - Per-symbol entry/stop/target levels
  - fomo_trade.py subprocess output
  - gocall.py starting confirmation
  - Error messages and position tracking

CONTINUOUS MONITORING:
  - Checks every INTERVAL seconds for new breakouts
  - Compares against already-traded symbols
  - Launches new trades for new breakouts
  - Shows status updates at each check
  - Runs until Ctrl+C

PERFORMANCE:
  - Check interval: 20 seconds (default)
  - Faster intervals = more frequent checks
  - API efficient (single file read per check)
  - Subprocess-based execution for parallelism

NOTES:
  - Requires valid Alpaca API credentials
  - Requires active trading account (paper or live)
  - Best run at market open (9:30 AM ET)
  - Requires find_daily_trend.py to generate breakout list
  - Entry based on yesterday's high (not current price)
  - Press Ctrl+C to stop monitoring
  - Started subprocesses continue running
  - Call options require options trading approval
  - Two-tier position management (stock + options)
""")
        return 0
