...
print(f"â•‘     FOMO Trade Bot for {SYMBOL:<22} â•‘")
print(f"â• â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•£")
print(f"â•‘ Entry Price:      ${ENTRY_PRICE:.2f}")
print(f"â•‘ Shares:           {NUM_STOCKS} (Target1: {FIRST_TARGET_QTY}, Target2: {SECOND_TARGET_QTY})")
print(f"â•‘ Stop Price:       ${STOP_PRICE:.2f}")
print(f"â•‘ Profit Target 1:  ${TARGET1_PRICE:.2f}")
print(f"â•‘ Profit Target 2:  ${TARGET2_PRICE:.2f}")
print(f"â•‘ Paper Trading:    {PAPER}")
...
if existing_position:
    print(f"âœ“ Existing position found: {existing_position.qty} {SYMBOL}")
    ...
    while True:
        try:
                ...
                if buy_orders:
                    ...
                    if awaiting_reentry_after_stop and crossed_above_entry:
                        ...
                        try:
                            ...
                            second_contract_stop_loss = None
                            awaiting_reentry_after_stop = False
                            print(f"âœ“ Re-entry BUY submitted. Order ID: {entry_order_id}")
                        except Exception as e:
                            print(f"âœ— Error submitting re-entry BUY: {e}")
                        ...
                        try:
                            ...
                            breakeven_stop_set = False
                            second_contract_stop_loss = None
                            print(f"âœ“ Entry BUY submitted. Order ID: {entry_order_id}")
                        except Exception as e:
                            print(f"âœ— Error submitting entry BUY: {e}")
                    else:
                        print(
            ...
            if not first_contract_sold and current_price <= current_stop_loss:
                print(f"\nâœ— STOP LOSS HIT at ${current_price:.2f}! Closing all {current_qty} {SYMBOL}...")
                try:
                    ...
                    print(f"âœ“ Position closed. Order ID: {close_response.id}")
                    print(f"\nâ•”â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—")
                    print(f"â•‘       STOPPED OUT                      â•‘")
                    print(f"â•‘ Loss: ${unrealized_pnl:.2f}")
                    print(f"â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•")
                    ...
                    print(f"âœ— Error executing stop loss: {e}")
            ...
            # Stop loss for remaining contract after first sale (at breakeven)
                ...
                print(f"\nâœ— BREAKEVEN STOP HIT at ${current_price:.2f}! Closing remaining {current_qty} {SYMBOL}...")
                try:
                    close_order = MarketOrderRequest(
                        symbol=SYMBOL,
                        qty=current_qty,
                    ...
                    print(f"âœ“ Position closed. Order ID: {close_response.id}")
                    print(f"\nâ•”â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—")
                    print(f"â•‘     BREAKEVEN STOP HIT                 â•‘")
                    print(f"â•‘ Final P&L: ${unrealized_pnl:.2f}")
                    print(f"â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•")
                    # Reset state for potential re-entry
                    first_contract_sold = False
                    breakeven_stop_set = False
            ...
            # Check if first profit target reached
                ...
                try:
                    ...
                    print(f"âœ“ Sold first contract. Order ID: {sell_response.id}")
                    ...
                    print(f"âœ“ Breakeven stop loss set at ${ENTRY_PRICE:.2f} for remaining contract")
                
                except Exception as e:
                    print(f"âœ— Error selling first contract: {e}")
            
            # Check if second profit target reached
            elif first_contract_sold and current_price >= TARGET2_PRICE:
                print(f"\nâœ“ SECOND PROFIT TARGET HIT at ${current_price:.2f}!")
