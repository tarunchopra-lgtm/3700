...
def get_current_week_put_option_symbol(ticker: str, reference_price: float | None = None) -> str:
    ...
    return _get_nearest_contract_symbol(trading_client, symbol, reference_price, ContractType.PUT)


def main() -> int:
    if len(sys.argv) not in (2, 3):
        print("Usage: python current_week_option_function_put.py <TICKER> [REFERENCE_PRICE]", file=sys.stderr)
        return 1

    try:
        ref_price = float(sys.argv[2]) if len(sys.argv) == 3 else None
        contract_symbol = get_current_week_put_option_symbol(sys.argv[1], ref_price)
